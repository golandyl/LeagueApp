...
export function StartTournamentButton({ leagueId, playerCount }: Props) {
  ...
  const [dayName,    setDayName]    = useState(
    () => `Tournament Day – ${new Date().toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}`
  )
  ...
  async function handleGenerate() {
    ...
    if (pErr || !players || players.length < numTeams) {
      setError(pErr?.message ?? `Need at least ${numTeams} players to start.`)
      setLoading(false)
      return
    }
    ...
    // 3. Generate balanced teams — pass player objects with _id attached
    ...
    if (teamsErr || !insertedTeams) {
      setError(teamsErr?.message ?? 'Failed to create teams.')
      setLoading(false)
      return
    }

    // 5. Insert team_players (real players only — ghosts are algorithm padding)
    ...
    // 6. Round-robin matches (every team pair plays once)
  ...
  return (
    <div className="rounded-2xl bg-slate-800 p-5 space-y-4">
      <h3 className="font-black text-white">New Tournament Day</h3>

      <div className="space-y-1.5">
        <p className="text-xs font-bold uppercase tracking-wide text-slate-400">Day Name</p>
        <input
          type="text"
          value={dayName}
      ...
      <div className="space-y-2">
        <p className="text-xs font-bold uppercase tracking-wide text-slate-400">Number of Teams</p>
        ...
        <p className="text-xs text-slate-500">
          {playerCount} players → ~{Math.ceil(playerCount / numTeams)} per team
          {ghostCount > 0 ? ` · ${ghostCount} ghost filler${ghostCount > 1 ? 's' : ''} added` : ''}
        </p>
      </div>
